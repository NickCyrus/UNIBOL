...
class contractsModel extends Model
{
    ...
    public function fechaEnd($format = true){

            if ($this->term && $this->initialdate){
                 return ($format) ? \Carbon\Carbon::parse($this->initialdate)->addDays($this->term)->format("d/m/Y") : \Carbon\Carbon::parse($this->initialdate)->addDays($this->term)->format("Y-m-d");
            }
